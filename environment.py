...
class FogEnvironment(gym.Env):
    ...
    def initialize_state(self):
        state = {
<<<<<<< HEAD
        'hosts': {i: {'CPU': host['CPU'], 'RAM': host['BW'], 'total_CPU': host['CPU'], 'total_RAM': host['BW']} for i, host in enumerate(self.infra_config['hosts']['configuration'])},
        'infra_links': {i: dict(link) for i, link in enumerate(self.infra_config['network']['topology'])},
        'applications': []
        }

        application_indices = list(range(len(self.app_configs['applications'])))
        random.shuffle(application_indices)

        global_comp_idx = 0
        for app_index in application_indices:
            app_config = self.app_configs['applications'][app_index]
            app_state = {
            'components': {i: {'CPU': comp['CPU'], 'RAM': comp['RAM'], 'deployed': False, 'app_id': app_index} for i, comp in enumerate(app_config['requirements'])},
...
<<<<<<< HEAD
                        raise ValueError(f"Host {host_id} does not have enough resources to deploy component {comp_id} of application {app_index} at initialization.")
=======
...
>>>>>>> 1678550bf925cc138a94f5f33500cc4e4e487090
        if action is not None:
            ...
            host = self.state['hosts'][host_id]
<<<<<<< HEAD
            energy_cost = self.calculate_energy_cost(host)
             # Normalize energy cost for single action
            normalized_energy_cost = energy_cost / (num_components or 1)
        else:
            # For final full calculation at the end of the episode
            normalized_energy_cost = self.calculate_total_energy_cost()
                ...
                if link['source'] == component_id or link['destination'] == component_id:
                    if app_state['components'][link['source']]['deployed'] and app_state['components'][link['destination']]['deployed']:
                        if app_state['paths'][link_id] is not None:
                            if app_state['latency_penalties'][link_id] > 0 :
        ...
        total_reward = 0
         ...
         # Use a different scaling factor to reduce sensitivity
        total_reward += 20 / (1 + normalized_energy_cost) 
...
<<<<<<< HEAD
        print(f"  Energy efficiency: {10 / (1 + normalized_energy_cost)  }")
=======
